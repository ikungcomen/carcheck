...
class EmployeeController extends CI_Controller {
    ...
    public function employee_eidt($id) {
        $data['gen'] = $this->DBhelper->get_list_gen();
        $data['province'] = $this->DBhelper->get_list_province();
        $data['district'] = $this->DBhelper->get_list_district();
        $data['subdistrict'] = $this->DBhelper->get_list_subdistrict();
        $this->load->view('include/header');
        $data['employee'] = $this->DBhelper->get_employee($id);
        $this->load->view('employee/employee_edit', $data);
        $this->load->view('include/footer');
    }

    public function save_employee($id) {
        $user = "admin";
        $gen = $this->input->post("gen");
        $name = $this->input->post("name");
        $lastName = $this->input->post("lastName");
        ...
        echo $subdistrict;//." - ".$district." - ".$province."<br>";

        $subName = $this->DBhelper->get_subName_xx($subdistrict);
        $disName = $this->DBhelper->get_disName($district);
        $proName = $this->DBhelper->get_proName($province);
        foreach ($subName->result() as $row) {
            $subName = $row->sub_name;
        ...
        echo $subName;//." = ".$disName." = ".$proName;

        //$date = date("Y-m-d");
        //$result = $this->DBhelper->save_employee($user, $gen, $name, $lastName, $employeeId, $phonenumber, $address, $subName, $disName, $proName, $postCode, $date, $id);
        ...
        //redirect('EmployeeController/employee_detail');
    }
